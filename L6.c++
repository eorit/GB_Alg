...
void Print_DArray(int** array, int len1, int len2) {  // печать одноменого массива
...
int* ODArray(int** array, int len1, int len2) {
    int* newODArray;
...
void fillRandom(int** arr, int len1, int len2, int border) {
    for (int i = 0; i < len1; i++) {
        for (int j = 0; j < len2; j++) {
            arr[i][j] = rand() % border;
        }
...
void ODArryaToBDArray(int** array1, int* array2, int len1, int len2) {
    int k = 0;
    for (int i = 0; i < len1; i++) {
        for (int j = 0; j < len2; j++) {
            array1[i][j] = array2[k];
